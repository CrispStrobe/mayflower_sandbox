...
def _translate_query(query: str) -> str:
    ...
    if (
        ...
        or "pg_indexes" in query.lower()
        or "pg_database" in query.lower()
    ):
    ...
    query = re.sub(r"=\s*ANY\s*\(\s*(\$\d+|\?)\s*\)", r"IN (\1)", query, flags=re.IGNORECASE)
...
class SqliteDatabase:
    ...
    def _init_db(self):
        ...
        with self._lock:
            ...
            try:
                self._conn.execute(
                    "ALTER TABLE sandbox_sessions ADD COLUMN parent_thread_id TEXT REFERENCES sandbox_sessions(thread_id) ON DELETE CASCADE"
                )
                self._conn.commit()
            except sqlite3.OperationalError:
