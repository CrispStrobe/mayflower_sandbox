        ...
        async with self.db.acquire() as conn:
            ...
            if session:
                ...
                if isinstance(expires_at, datetime) and expires_at < datetime.now():
                    raise SessionExpiredError(f"Session {thread_id} expired at {expires_at}")
            ...
            logger.info(
                f"Created new session for thread {thread_id} (VFS: {effective_vfs_id}), expires {expires_at}"
            )
            return self._safe_dict(session)
        ...
        async with self.db.acquire() as conn:
            ...
            try:
                ...
                if "parent_thread_id" in str(e).lower() or "no column" in str(e).lower():
                    raise RuntimeError(
                        "Database migration for VFS snapshots has not been applied. Please run migrations/004_vfs_snapshots.sql"
                    ) from e
                raise
            ...
            logger.debug(
                f"Snapshot {snapshot_id}: copied files from {parent_vfs_id}, result: {res}"
            )
    ...
    async def restore_snapshot(self, snapshot_id: str, target_thread_id: str | None = None) -> str:
        ...
        async with self.db.acquire() as conn:
            snap = await conn.fetchrow(
                "SELECT * FROM sandbox_sessions WHERE thread_id = $1", snapshot_id
            )
            if not snap:
            ...
            target_session = await conn.fetchrow(
                "SELECT * FROM sandbox_sessions WHERE thread_id = $1", target
            )
            ...
            logger.debug(
                f"Restoring snapshot {snapshot_id} (VFS: {snapshot_vfs_id}) to target {target} (VFS: {target_vfs_id})"
            )
